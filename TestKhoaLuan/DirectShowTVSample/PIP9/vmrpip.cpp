...
void ProcAmpAdjust(void)
{

    // Display a custom dialog to adjust brightness, hue, saturation
    CreateDialog(ghInst, MAKEINTRESOURCE(IDD_PROCAMP),
              ghApp,  (DLGPROC) ProcAmpAdjustProc);
    ...
    switch (message)
    {
        case WM_INITDIALOG:
			...
			if (pMix) 
            {
				...
				if (SUCCEEDED(hr)) 
                {
                    // For each ProcAmp control, check if it's supported. If so, get the
                    // range and set the slider value. Otherwise, disable the control
                    ...
                    for (prop = ProcAmpControl9_Brightness,
                         ...
                         prop <<= 1, pVal++)
                    {
                        HWND hSlider = GetDlgItem(hWnd, IDC_SLIDER_BASE + prop);
                        ...
                        if (myProcAmpControl.dwFlags & prop)
                        {
                            ...
                            myProcAmpControlRange.dwProperty = (VMR9ProcAmpControlFlags)prop;
                            hr = pMix->GetProcAmpControlRange(streamNum, &myProcAmpControlRange);
                            if SUCCEEDED(hr) 
                            {
                                // OK, we have the value and the ranges for the brightness
                                // So let's normalize for a 1 - 100 scale
                                float totalRange = myProcAmpControlRange.MaxValue - myProcAmpControlRange.MinValue;
                                float absoluteValue = *pVal - myProcAmpControlRange.MinValue;
                                double ratio = (absoluteValue/totalRange) * 100.0;
                                long radical = (long)ratio;	// nasty conversion, should be ok.

                                // Now we've got to set the slider to reflect its value in the total range
                                SendMessage(hSlider, TBM_SETPOS, TRUE, radical);
                            }
                        }
                        else
                        {
                            // Graphics driver does not support this property. Disable
                            // the slider.
					...
					// We failed the GetProcAmpControl call.  Disable all sliders.
					EnableWindow(GetDlgItem(hWnd, IDC_SLIDER_BRIGHTNESS),FALSE);
					EnableWindow(GetDlgItem(hWnd, IDC_SLIDER_CONTRAST),FALSE);
					EnableWindow(GetDlgItem(hWnd, IDC_SLIDER_HUE),FALSE);
					EnableWindow(GetDlgItem(hWnd, IDC_SLIDER_SATURATION),FALSE);
				}
            }
            return TRUE;
		...
		case WM_HSCROLL:
            ...
            if SUCCEEDED(hr) {
                ...
                theValue = theValue + myProcAmpControlRange.MinValue; // Add the offset back in
                ...
                hr = pMix->SetProcAmpControl(streamNum, &myProcAmpControl);
    ...
    switch(message)
    {
        ...
        case WM_COMMAND:
            ...
            switch(wParam)
            {
				...
				case ID_EFFECTS_PROCAMPADJUSTMENT:
					ProcAmpAdjust();
					break;
